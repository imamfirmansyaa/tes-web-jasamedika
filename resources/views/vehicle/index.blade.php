...
@section('content-top')
    ...
    <div class="card-header border-0">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label font-weight-bold font-size-h4 text-dark-75">Data Mobil</span>
        </h3>
        <div class="card-toolbar">
            <a href="{{ route('vehicle.create')}}" class="btn btn-primary font-weight-bolder font-size-sm">
    ...
    <div class="card-body py-0">
        ...
        <!--begin::Table-->
        <table class="table table-striped table-row-bordered gy-5 gs-7 border rounded" id="kt_datatable-literature">
            ...
            <thead>
                <tr>
                    ...
                    <th>Merk Mobil</th>
                    <th>Model Mobil</th>
                    <th id="column-1">Nomor Plat</th>
                    <th id="column-2">Tarif Sewa</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                @foreach ($vehicles as $vh)
                    <tr>
                        ...
                        <td> {{ $vh->tyoe_vehicle }} </td>
                        <td> {{ $vh->model_vehicle }} </td>
                        <td> {{ $vh->police_number }} </td>
                        <td> {{ $vh->rental_rates }} </td>
                        <td style="vertical-align: top">
                        </td>
                        <td nowrap="nowrap">
                            <a href="/vehicle/delete/{{ $vh->id }}">
...
@section('script')
    <script>
        function deleteLiterature(id, name) {
            $('#modal-name').html(name);
            $('#modal-confirm_delete').attr('onclick', `confirmDeleteLiterature(${id})`);
        ...
        function confirmDeleteLiterature(id) {
            $.ajaxSetup({
                headers: {
